...
namespace OzonEdu.MerchApi.Tests.CommandHandlers
{
    public class CreateMerchRequestCommandHandlerTests
    {
        ...
        public async void Handle_WhenSuccess()
        {
            ...
            merchRequestService
                .Setup(x
                    => x.CreateMerchRequestAsync(It.IsAny<Email>(), It.IsAny<Email>(), It.IsAny<MerchType>(),
                        It.IsAny<MerchRequestMode>(), It.IsAny<CancellationToken>()))
                .ReturnsAsync(MerchRequest.Create(new EmployeeId(employee.Id), MerchRequestMode.Auto,
                    DateTimeOffset.UtcNow, Email.Create("manager@ozon.ru")));
            ...
            var result = await handler.Handle(
                new CreateMerchRequestCommand("zxcvb@asdasd.ru", "zxcvb@asdasd.ru", MerchType.VeteranPack,
        ...
        public async void Handle_ValidationError()
        {
            ...
            var result = await handler.Handle(
                new CreateMerchRequestCommand("incorrect", "coorect@email.yes", MerchType.VeteranPack,
                    MerchRequestMode.ByRequest),
                CancellationToken.None);
